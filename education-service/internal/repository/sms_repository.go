...
func (r SmsRepository) SetSmsTemplate(req *pb.SetSmsTemplateRequest, companyId string) (*pb.AbsResponse, error) {
	...
	case "update":
		...
		if req.ActionName == "ACTION" {
			...
			`, pq.Array(textParts), req.SmsCount, isActive, req.InsufficientBalanceResendCount, companyId, req.ActionType)
		} else {
			_, err = r.db.Exec(`
				UPDATE sms_template
	...
	case "create":
		...
		if req.ActionName == "ACTION" {
			...
			`, companyId, pq.Array(textParts), req.SmsCount, isActive, req.InsufficientBalanceResendCount, req.ActionType)
		} else {
			_, err = r.db.Exec(`
